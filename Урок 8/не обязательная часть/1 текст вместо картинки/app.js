...
function handleClick(clickedButtonEvent) {
    ...
    const buttonName = card.button.innerText;

    if (buttonName === "Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (buttonName === "Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    const text = "Один человек — это все, что стоит между человечеством и величайшей угрозой всему живому. Вспомните легенду: капитан Шепард возвращается в новом издании знаменитой трилогии Mass Effect — Mass Effect™ издание Legendary. Исследуйте уголки невероятно богатой вселенной, где все ваши действия и решения будут иметь последствия.";
